...
# NF: 表示字段数，在执行过程中对应于当前的字段数, 即当前行被分割成了几列
# /^-/ 匹配以 - 开头的行
# 空格：字符串拼接
# [\\\"'] 匹配反斜杠，双引号和单引号
function exec_cmd() {
...
function have_sudo_access() {
	...
	if [[ -z "${HAVE_SUDO_ACCESS-}" ]]; then
		echo -e "${BOLD}${BLUE}==> ${CYAN}Checking sudo access (press ${YELLOW}Ctrl+C${CYAN} to run as normal user).${RESET}" >&2
		# [*] 取全部字段
	...
	if [[ "${HAVE_SUDO_ACCESS}" == "0" ]]; then
		echo -e "${BOLD}${GREEN}==> ${MAGENTA}sudo access available.${RESET}" >&2
	else
		echo -e "${BOLD}${RED}==> ${CYAN}sudo access unavailable.${RESET}" >&2
	fi
...
# NOTE change APT sources and install packages
if have_sudo_access; then
	exit 0
	# NOTE Setup APT sources
	# < <(find -L /etc/apt -type f -name '*.list') 传递文件名
	# < <(echo "/etc/apt/sources.list") 传递文件名
	# < <<EOF /etc/apt/sources.list EOF 传递文件名
	# < /etc/apt/sources.list 传递文件内容
	if ${SET_MIRRORS}; then
...
EOS
	...
	# NOTE add apt-repositiory
	exec_cmd 'sudo apt-get update'
	# * software-properties-common 提供 add-apt-repositiory 工具可以安装个人库ppa(personal package archive)
	...
	# NOTE Install and setup shells, zsh
	...
	# * fd from github
	# NOTE download and install demo
	if [[ $? -ne 0 || -z "$(apt-cache search '^fd-find$' --names-only)" ]]; then
		# v8.3.2
		LATEST_FD_VERSION="$(get_latest_version "sharkdp/fd")"
		# wget -N, –-timestamping 不要重新下载文件除非比本地文件新 -P, –-directory-prefix=PREFIX 将文件保存到目录 PREFIX/…
...
#                                   Homebrew                                  #
# --------------------------------------------------------------------------- #
# NOTE Install and setup Homebrew
if ${SET_MIRRORS}; then
...
#                                     ruby                                    #
# --------------------------------------------------------------------------- #
# NOTE Configurations for RubyGems
...
# * 查包
# *  conda-zsh-completion, colored-man-pages
plugins=(
	ubuntu
