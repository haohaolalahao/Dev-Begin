...
if [[ "${SET_MIRRORS}" =~ (yes|Yes|YES|true|True|TRUE) ]]; then
	...
	if [ -t 0 ] && [ -t 1 ]; then
		while true; do
			...
			read -n 1 -p "$(echo -e "${BOLD}${CYAN}Do you wish to set the source of package managers ${BLUE}(${PACKAGE_MANAGER}, Conda, and Pip)${CYAN} to the open source mirrors ? ${WHITE}[y/N]: ${RESET}")" answer
			# -n sting 不为空 返回 true
			if [[ -n "${answer}" ]]; then
				echo
			else
